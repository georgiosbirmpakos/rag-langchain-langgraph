#!/usr/bin/env python3
"""
Windows-Compatible Vector Database Updater
Updates the Pinecone vector database with fresh content from Gazzetta.gr every hour.
This version removes emojis for better Windows compatibility.
"""
...
class VectorDBUpdater:
    ...
    def __init__(self):
        ...
        self.logger.info("[INIT] Initializing Vector Database Updater...")
        ...
        self.logger.info("[SUCCESS] Vector Database Updater initialized successfully!")
    ...
    def _load_environment(self):
        ...
        try:
            ...
            self.logger.warning("[WARNING] python-dotenv not installed. Make sure to set environment variables manually.")
        ...
        self.logger.info("[SUCCESS] Environment variables loaded")
    ...
    def _init_embeddings(self):
        ...
        self.logger.info("[SUCCESS] Embeddings model initialized")
    ...
    def _init_vector_store(self):
        ...
        self.logger.info("[SUCCESS] Vector store initialized")
    
    def load_fresh_content(self) -> List[Document]:
        """Load fresh content from Gazzetta.gr"""
        self.logger.info("[LOAD] Loading fresh content from Gazzetta.gr...")
        ...
        for url in self.greek_derby_urls:
            try:
                self.logger.info(f"[LOAD] Loading: {url}")
                ...
                if not docs or all(len(doc.page_content.strip()) < 100 for doc in docs):
                    self.logger.info(f"[LOAD] No content found with selectors, trying without filtering...")
                ...
                if valid_docs:
                    ...
                    self.logger.info(f"[SUCCESS] Found {len(valid_docs)} valid documents from {url}")
                else:
                    self.logger.warning(f"[WARNING] No valid content found from {url}")
                
                # Be respectful to the server
                time.sleep(2)
                
            except Exception as e:
                self.logger.error(f"[ERROR] Error loading {url}: {e}")
                continue
        
        # If still no content, try fallback approach
        if len(all_docs) == 0 or all(len(doc.page_content.strip()) < 100 for doc in all_docs):
            self.logger.warning("[WARNING] Trying fallback approach with requests...")
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.logger.info("[SUCCESS] Added fallback document from requests")
            except Exception as e:
                self.logger.error(f"[ERROR] Fallback approach also failed: {e}")
        
        self.logger.info(f"[SUCCESS] Loaded {len(all_docs)} documents from {len(successful_urls)} URLs")
        return all_docs
    
    def process_and_store_content(self, docs: List[Document]) -> int:
        """Process documents and store them in the vector database"""
        if not docs:
            self.logger.warning("[WARNING] No documents to process")
            return 0
        
        self.logger.info("[PROCESS] Processing and storing content...")
        ...
        try:
            ...
            self.logger.info(f"[SUCCESS] Successfully stored {len(splits)} chunks in vector database")
            return len(splits)
        except Exception as e:
            self.logger.error(f"[ERROR] Error storing documents: {e}")
            return 0
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"[ERROR] Error getting database stats: {e}")
            return {}
    ...
    def cleanup_old_content(self, hours_threshold: int = 24):
        ...
        self.logger.info(f"[INFO] Cleanup not implemented yet. Would clean content older than {hours_threshold} hours")
        pass
    
    def run_update(self) -> bool:
        """Run a complete update cycle"""
        start_time = time.time()
        self.logger.info("[UPDATE] Starting vector database update cycle...")
        
        try:
            # Get current stats
            stats_before = self.get_database_stats()
            self.logger.info(f"[STATS] Database stats before update: {stats_before}")
            
            # Load fresh content
            docs = self.load_fresh_content()
            
            if not docs:
                self.logger.error("[ERROR] No content loaded. Update failed.")
                return False
            ...
            if chunks_added == 0:
                self.logger.error("[ERROR] No chunks were added. Update failed.")
                return False
            
            # Get updated stats
            stats_after = self.get_database_stats()
            self.logger.info(f"[STATS] Database stats after update: {stats_after}")
            
            # Calculate duration
            duration = time.time() - start_time
            self.logger.info(f"[SUCCESS] Update completed successfully in {duration:.2f} seconds")
            self.logger.info(f"[SUCCESS] Added {chunks_added} new chunks to the database")
            
            return True
            
        except Exception as e:
            self.logger.error(f"[ERROR] Update failed with error: {e}")
            return False
...
def main():
    ...
    if success:
        print("[SUCCESS] Vector database update completed successfully!")
        sys.exit(0)
    else:
        print("[ERROR] Vector database update failed!")
        sys.exit(1)
